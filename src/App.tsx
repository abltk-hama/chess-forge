...
function SimulationView({ defs, setup, preset }: { defs: Definition[]; setup: Setup; preset: Preset }) {
  ...
  const start = () => {
    ...
    worker.postMessage({ defs, setup, preset, options: { games, whiteDifficulty, blackDifficulty, thinkTimeMs, swapSides, maxPlies, seed: Date.now() & 0x7fffffff } });
  };
  ...
  return <section><h2>AIバランスシミュレーション</h2><div className="panel simulation-controls">
    ...
    <label>思考時間<select aria-label="AI思考時間" value={thinkTimeMs} onChange={(e) => setThinkTimeMs(Number(e.target.value) as 10 | 25 | 100)}><option value="10">高速（10ms）</option><option value="25">標準（25ms）</option><option value="100">精密（100ms）</option></select></label>
